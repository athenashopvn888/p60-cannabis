...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | P60 Cannabis York`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://p60cannabis.com/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | P60 Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://p60cannabis.com/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://p60cannabis.com${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://p60cannabis.com/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://p60cannabis.com/item/${item.slug}`
      }
    ]
  };
}
